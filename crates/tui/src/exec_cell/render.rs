...
pub(crate) fn truncated_tool_output_preview(
    preview: &str,
    width: u16,
    max_rows: usize,
    line_limit: usize,
    ...
    let raw_output = output_lines(
        Some(&CommandOutput {
            exit_code: 0,
            aggregated_output: preview.to_string(),
    ...
    let wrap_opts = RtOptions::new(wrap_width as usize).word_splitter(WordSplitter::NoHyphenation);
    let mut wrapped_output: Vec<Line<'static>> = Vec::new();
    for line in &raw_output.lines {
        push_owned_lines(
            &adaptive_wrap_line(line, wrap_opts.clone()),
            &mut wrapped_output,
        );
    }

    let prefixed_output =
        prefix_lines(wrapped_output, Span::from("  └ ").dim(), Span::from("    "));
