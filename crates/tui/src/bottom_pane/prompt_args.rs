...
/// starts after trimming leading whitespace.
pub fn parse_slash_name(line: &str) -> Option<(&str, &str, usize)> {
    ...
    let rest_leading_trimmed = rest_untrimmed.trim_start();
    let rest = rest_leading_trimmed.trim_end();
    let rest_start_in_stripped =
        name_end_in_stripped + (rest_untrimmed.len() - rest_leading_trimmed.len());
