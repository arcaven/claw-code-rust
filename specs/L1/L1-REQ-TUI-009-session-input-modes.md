...
The initial TUI input modes are Default Input Mode, Shell Mode, and Plan Mode. Default Input Mode is the normal chat input state and should not display a mode label. Shell Mode and Plan Mode are non-default input modes and must be visible while active.
...
- A user returns to Default Input Mode and does not see an unnecessary default mode label.
...
- When Default Input Mode is active, the TUI must not display a default mode label.
...
- Given the TUI composer is in Default Input Mode, when the bottom status line renders, then no default input mode label is shown.
...
- What exact label text should be used for Shell Mode and Plan Mode?
