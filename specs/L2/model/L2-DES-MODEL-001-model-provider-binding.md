...
- `canonical_model_slug`: reference to an effective catalog model slug.
...
Default selection is not a binding field. It is stored under `[defaults]` so the default can be changed without rewriting the binding record.

`display_name` is mutable display metadata. It must not be used as a stable identifier, provider API model name, traceability key, or configuration reference. The stable references remain `binding_id`, `canonical_model_slug`, and `provider_id`; the provider request uses `model_name`. Runtime turn metadata keeps `model` as the catalog slug and `request_model` as the provider request model, even when both strings are identical.
...
`UserProvider` and `ModelProviderBinding` are durable configuration records. Onboarding and model setup create or update these records, while model capability metadata comes from the effective catalog.
...
User-scoped providers and bindings
        +
Workspace-scoped providers and bindings
        ↓
...
| related-to | L2-DES-APP-002 | 2 | specs/L2/app/L2-DES-APP-002-configuration-precedence.md | Configuration precedence resolves durable provider and binding records. |
| related-to | L2-DES-APP-005 | 2 | specs/L2/app/L2-DES-APP-005-config-toml-schema.md | Defines concrete TOML fields for persisted providers, bindings, defaults, and user-scoped `auth.json` credential storage. |
| specified-by | L3-BEH-PROVIDER-001 | 3 | specs/L3/provider/L3-BEH-PROVIDER-001-model-resolution.md | L3 defines model catalog loading, provider validation, binding resolution, session model selection, and provider request construction. |
