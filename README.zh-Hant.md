...
**開源、模型中立的 Agent Desktop / Runtime，面向私有化、企業內網和 OpenAI 相容模型環境。可接入 DeepSeek、Qwen、Kimi、Anthropic 相容 API、本地閘道或自訂模型端點。**
...
[為什麼選擇 Devo](#為什麼選擇-devo) · [功能](#功能) · [已測試模型](#已測試模型) · [已測試平台](#已測試平台) · [安裝](#安裝) · [快速開始](#快速開始) · [文件](#docs)
...
- **Desktop 與 CLI 雙入口** - 用 Desktop app 完成上手和日常編碼，也可以在需要
  終端機原生自動化時使用 CLI/TUI。
...
Devo 有兩種安裝形態。你可以只安裝 Desktop app，只安裝終端機原生的 TUI/CLI，
也可以在同一台機器上同時安裝兩者。
