...
**开源、模型中立的 Agent Desktop / Runtime，面向私有化、企业内网和 OpenAI 兼容模型环境。可接入 DeepSeek、Qwen、Kimi、Anthropic 兼容 API、本地网关或自定义模型端点。**
...
[为什么选择 Devo](#为什么选择-devo) · [功能](#功能) · [已测试模型](#已测试模型) · [已测试平台](#已测试平台) · [安装](#安装) · [快速开始](#快速开始) · [文档](#docs)
...
- **Desktop 与 CLI 双入口** - 用 Desktop app 完成上手和日常编码，也可以在需要
  终端原生自动化时使用 CLI/TUI。
...
Devo 有两种安装形态。你可以只安装 Desktop app，只安装终端原生的 TUI/CLI，
也可以在同一台机器上同时安装两者。
